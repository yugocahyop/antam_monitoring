...
import 'package:antam_monitoring/home/widget/content_support/widget/supportPanel.dart';
...
class Content_support extends StatefulWidget {
  Content_support(
      {super.key,
  ...
  @override
  State<Content_support> createState() => _Content_callState();
}

class _Content_callState extends State<Content_support> {
  var alarm = [
    {
      "title": "Status",
      "isActive": true,
  ...
  getSupportData(int offset) async {
    if (mounted) {
      setState(() {
        isLoading = true;
      });
    ...
    final r =
        await api.callAPI("/support/find", "POST", "{}", true);

    if (r["error"] == null) {
      if (kDebugMode) {
        print("support data: ${r['files']}");
      }
      final files = (r["files"] as List<dynamic>);
      if (files.isNotEmpty) {
        callData.clear();
        // callData = r["data"];

        for (var i = 0; i < files.length; i++) {
          callData.add({"name": files[i] as String });
        }
      }
